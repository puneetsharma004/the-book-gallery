  ...
  return (
    <main className={`min-h-screen ${BG_COLOR} p-6`}>
        ...
        <header className={`text-center mb-10 bg-white p-6 rounded-xl shadow-lg border ${BORDER_COLOR}`}>
          ...
          <Button
            onClick={() => {
              ...
              import("react-hot-toast").then(({ default: toast }) => toast.success("Profile link copied! 🔗"));
            }}
            className="bg-amber-600 hover:bg-amber-700 text-white text-sm px-4 py-2 rounded-md mr-2"
          >
            Copy Profile Link 🔗
          </Button>
          ...
          <Button
            ...
            className="border-amber-400 text-amber-700 hover:bg-amber-100 text-sm px-4 py-2 rounded-md"
          >
            Show QR Code 🧾
          </Button>
      ...
      <Dialog open={showQR} onOpenChange={setShowQR}>
        <DialogContent className="max-w-sm p-6 bg-white rounded-xl shadow-xl border border-amber-200">
          <DialogHeader>
            <DialogTitle className="text-center text-lg font-semibold text-stone-800">
